...
$year=date("Y");
	...
	$dataHoliday = mysql_query("SELECT `date`  FROM `leavecalendar` where `year` = '$year'",$con) or die(mysql_error());
	$holidayArr = array();
...
<?php
if($flag == 0){
$getData = mysql_query("SELECT * FROM `leaverequest` WHERE `id` = '$lid'",$con) or die(mysql_error());
$row = mysql_fetch_array($getData);
?>
