...
<?php
	...
	/*
	 * $getLead = mysql_query("SELECT COUNT(`id`) FROM `team` WHERE `leader` = '$empid'",$con) or die(mysql_error());
	...
	*/
	 ...
	 * */
		...
		$getData = mysql_query("SELECT employee.id,employee.name,employee.salaryIdNew,employee.designation FROM employee WHERE employee.id = '$empid' AND employee.delete = '0' AND employee.empstatus = '2'",$con) or die(mysql_error());
		
	$getCount = mysql_query("SELECT `id` FROM `kpi` WHERE `month` = '$smonth' AND `year` = '$syear'",$con) or die(mysql_error());
	...
	<table cellpadding="5" cellspacing="0" class="fetch" style="text-align: center" width="100%">
		...
		<?php
			...
			while($row = mysql_fetch_array($getData))
			{
				$userids .= $row[0].",";
				$salpid = $row[2];
				$getsalary = mysql_query("SELECT `variablevalue` FROM `salary_structure_relation_new` WHERE `profileid` = '$salpid' AND `variableid`='7'",$con) or die(mysql_error());
					...
					<?php
						...
						while($rowParams = mysql_fetch_array($getParams))
						{
							...
							if($marksCount > 0)
							{
								...
								$getMarks = mysql_query("SELECT `marks` FROM `kpi` WHERE `employee` = '$row[0]'  AND `kpiparameter` = '$kpiid' AND `month` = '$smonth' AND `year` = '$syear'",$con) or die(mysql_error());
        ...
        <?php if(in_array('u_Tkpi',$thisper))
        {
            ?>
		<tr>
			<td colspan="<?php echo $g?>" style="text-align: center">
				<button class="button green" onclick="SaveData('kpi/saveMass?userids=<?php echo $userids?>&amp;g=<?php echo $g;?>&amp;smonth=<?php echo $smonth;?>&amp;syear=<?php echo $syear;?>','kpi','<?php echo $j?>','','','myResp','1')">
					Save</button></td>
		</tr>
            <?php
        }
