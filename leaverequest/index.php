<?php
	...
	$travelid = $_GET['id'];
	$sqlLeave = mysql_query("SELECT `prefix`,`id`,`name`,`type` FROM `leavetype` WHERE `delete` = '0'",$con) or die(mysql_error());
	...
	while($getLeave = mysql_fetch_array($sqlLeave))
	{
		...
		if($type!=1)
		{
			...
			$selectLeaveAll .='<option value="'.$prefix.'">'.$name.'</option>';
			// $selectLeaveAll .='<option value="'.$prefix.'**LWP">'.$name.' + LWP</option>';
	...
	$selectLeaveAll .= '<option value="LWP">Leave Without Pay(LWP)</option>';
?>
	...
	<div class="add-new green-border">
		...
		<table cellpadding="0" cellspacing="0" width="100%">
			...
			<tr>
				...
				<td>
					<input name="req" id="levras" type="hidden" value="<?=date("Y-n-j")?>"/>
			...
			<tr>
				...
				<td><input name="req" id="levr2" type="" readonly="readonly" class="inputCalendar" style="width:200px" onclick="openCalender('calenderidl1','levr2')" onblur="if($('#levr1').val()==''){alert('Please Select From Date');$('#calenderidl1').hide();$('#levr2').val('');}else{}"/>
			...
			<tr>
				...
				<td>
					<select name="req" id="levr0" class="input drop-down large" onchange="if($('#levr2').val()=='' || $('#levr2').val()=='0'){alert('Please Select To Date');$('#levr0').val('');}date_reducechecker('#levras','#levr1','#levr0','#levr2');">
			...
			<tr>
				...
				<td>
					<select name='req' id="levr4" class="input drop-down large" name="Select2" onchange="checkLeavestatus(this.value,'<?php echo $hrmloggedid;?>',1)" >
						<?php
							if($row[6] == 1)
								echo $selectLeave;
							else
			...
			<tr>
				<td colspan="4" style="text-align:center">
					<button class="button green" onclick="SaveData('leaverequest/save','levr','6','','','couRespView','1');ToggleBox('manipulateContent','none','');ToggleBox('viewContent','block','')"><i class="save-icon"></i>Apply</button>
