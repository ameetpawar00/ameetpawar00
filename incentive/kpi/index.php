...
$id = $_GET['id'];
$i = $_GET['i'];
$getData = mysql_query("select * from `incentivekpi` where `id` = '$id'",$con)or die(mysql_error());
$rowData = mysql_fetch_array($getData);
$url = "incentive/kpi/save?id=$id&i=$i";
	if($rowData['type'] == "1")
	{
		$value= "Flat Amount";
	}
...
<tr><th >Designation  *</th>
	<td >
	<select class="input drop-down large" id="incea0"  title="isNotNull" name="req" onchange="incentiveexist('incentive/kpi/check.php?id=<?php echo $id?>','couResp','incea0','incea2','incea3','incea6')">
	...
	<th >KPI*</th>
	<td >
	<select  class="input drop-down large" id="incea6"  title="isNotNull" name="req" onchange="incentiveexist('incentive/kpi/check.php?id=<?php echo $id?>','couResp','incea0','incea2','incea3','incea6')">
	<option value=""> Select A KPI </option>
	<?php
	$getkpi = mysql_query("select `id`,`name` from `kpiparameters` where `delete` = '0' and `id` != '1' order by `name` asc",$con) or die(mysql_error());
	while($rowkpi = mysql_fetch_array($getkpi))
	{
	?>
	<option <?php if($rowData['kpiid'] == $rowkpi[0]) {echo "selected=selected" ;} ?>  value="<?php echo $rowkpi[0]?>"><?php echo $rowkpi[1]?></option>
	<?php
	}
	?>
	</select>
...
<th>From *</th>
<td><input type="text" title="isNotNull" value="<?php echo $rowData['from']?>" id="incea2" class="input medium" onkeypress="return chkDecimal(event,'showError0')" name="req" onblur="incentiveexist('incentive/kpi/check.php?id=<?php echo $id?>','couResp','incea0','incea2','incea3','incea6')">
<span id="showError0" class="showError"></span>

</td>
<th>To *</th>
<td>
<input type="text" title="isNotNull" value="<?php echo $rowData['to']?>" id="incea3" class="input medium" onkeypress="return chkDecimal(event,'showError1')" name="req" onblur="incentiveexist('incentive/kpi/check.php?id=<?php echo $id?>','couResp','incea0','incea2','incea3','incea6')" style="height: 24px" >
...
<td><input type="text" title="isNotNull" value="<?php echo $rowData['value']?>" id="incea5" class="input medium" onkeypress="return chkDecimal(event,'showError2')" name="req" >
...
<th>Action *</th>
		<td>
		<select  class="input drop-down large" id="incea1"  title="isNotNull" name="req">
...
<button class="button green" onclick="SaveData('<?php echo $url?>','incea','7','<?php echo $i;?>','','couResp','<?php echo $what?>');ToggleBox('manipulateContent','none','');ToggleBox('viewContent','block','')"><i class="save-icon"></i><?php if($edit == 1){ ?> <span> Update </span> <?php } else { ?> <span> Save </span> <?php } ?> </button>
...
<div class="blue awesome small" onclick="SaveData('<?php echo $url?>','incea','7','','','couResp','1')">Save</div>
