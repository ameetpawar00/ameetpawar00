...
$id = $_GET['id'];
$i = $_GET['i'];
$getData = mysql_query("select * from `incentiveattendance` where `id` = '$id'",$con)or die(mysql_error());
$rowData = mysql_fetch_array($getData);
$url = "incentive/attendance/save?id=$id&i=$i";
	if($rowData['type'] == "1")
	{
		$value= "Flat Amount";
	}
...
<tr><th >Designation  *</th>
	<td >
	<select class="input drop-down large" id="incea0"  title="isNotNull" name="req" onchange="incentiveexist('incentive/attendance/check.php?id=<?php echo $id?>','couResp','incea0','incea2','incea3','')">
	...
	?>
		...
		<th>Action *</th>
		<td>
		<select class="input drop-down large" id="incea1"  title="isNotNull" name="req" onchange="incentiveexist('incentive/attendance/check.php?id=<?php echo $id?>','couResp','incea0','incea2','incea3','')">
...
<th>From *</th>
<td><input type="text" title="isNotNull" value="<?php echo $rowData['from']?>" id="incea2" class="input medium" onkeypress="return chkDecimal(event,'showError0')" name="req" onblur="incentiveexist('incentive/attendance/check.php?id=<?php echo $id?>','couResp','incea0','incea2','incea3','')">
<span id="showError0" class="showError"></span>

</td>
<th>To *</th>
<td><input type="text" title="isNotNull" value="<?php echo $rowData['to']?>" id="incea3" class="input medium" onkeypress="return chkDecimal(event,'showError1')" name="req" onblur="incentiveexist('incentive/attendance/check.php?id=<?php echo $id?>','couResp','incea0','incea2','incea3','')" >
<input type="hidden" id="incea6" value="0">
