<?php
    ...
    $sql = "SELECT * FROM `post` WHERE `delete`='0'";
    $getData = mysql_query($sql,$con) or die(mysql_error());
    $Num_Rows = mysql_num_rows($getData);
    $Per_Page = 25;   // Per Page
    include('../../pagination/pagination.php');
    $folder= 'masters/post/view';
    $title = 'post';

    //$getData = mysql_query("SELECT * FROM `post` WHERE `delete` = '0' AND `id` != '1' ORDER BY `name` ASC",$con) or die(mysql_error());
?>
<div id="myTitle">
    <div class="title">View All Post</div>
    <div class="strip">
        <span>Dashboard</span>
        <span>Post</span>
        <span>View</span>
...
<table width="100%" cellpadding="0" cellspacing="0" id="headerTable">
    <tr>
        ...
        <td style="width:70%" align="right">
            ...
            {
                ?>
                <button class="button blue" onclick="getModule('masters/post/index','manipulateContent','viewContent','Asset')"> <i class="plus"></i>New Post</button>&nbsp;
                <?php
            }
            ?>
            ...
            {
                ?>
                <button class="button red" onclick="deleteData('post','Post')"> <i class="delete-icon"></i>Delete</button>&nbsp;
                <?php
            }
            ?>
...
<div style="height:350px;overflow:auto" id="mainDivId">
    <table width="100%" cellpadding="5" cellspacing="0"  class="fetch" id="mytable">
        <tr><th style="width:5%"><input id="mainChk" name="Checkbox1" onclick="chkAll('chBx','mainChk')"type="checkbox" /></th>
            <th >Name</th><th style="height: 30px">Description</th>
            <th >Modified</th>
        </tr>
        <?php
            $i = 1;
            ...
            while($row =mysql_fetch_array($values))
            {
                ...
                <tr  class="d<?php echo $i%2?>"  id="fetchrow<?php echo $i?>">
                    ...
                    {
                        ?>
                        <td class="link-blue" onclick="getModule('masters/post/edit?id=<?php echo $row[0]?>&i=<?php echo $i?>','manipulateContent','viewContent','Post')"><?php echo $row['name'] ?></td>
                        <?php
                    }
                    else
                    {
