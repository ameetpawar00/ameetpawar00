...
$sql = "SELECT * FROM `employeestatus` WHERE `delete`='0' AND `id` != '1'";
$getData = mysql_query($sql,$con) or die(mysql_error());
$Num_Rows = mysql_num_rows($getData);
	$Per_Page = 25;   // Per Page
	include('../../pagination/pagination.php');
	$folder= 'masters/employeestatus/view';
	$title = 'Status';

//$getData = mysql_query("SELECT * FROM `employeestatus` WHERE `delete` = '0' AND `id` != '1' ORDER BY `name` ASC",$con) or die(mysql_error());
?>
<div id="myTitle">
<div class="title">View All Status</div>
<div class="strip">
<span>Dashboard</span>
...
<?php if(in_array('a_MEStatus',$thisper)) 
{
?>
<button class="button blue" onclick="getModule('masters/employeestatus/index','manipulateContent','viewContent','Employee Status')"> <i class="plus"></i>New Employee Status</button>&nbsp;
<?php 
} 
?>
<?php if(in_array('d_MEStatus',$thisper)) 
{
?>
<button class="button red" onclick="deleteData('employeestatus','employeestatus')"> <i class="delete-icon"></i>Delete</button>&nbsp;
<?php 
} 
?>
...
<?php if(in_array('u_MEStatus',$thisper)) 
{
?>
