...
$sql = "SELECT * FROM `sourceofhire` WHERE `delete`='0' AND `id` != '1'";
$getData = mysql_query($sql,$con) or die(mysql_error());
$Num_Rows = mysql_num_rows($getData);
	$Per_Page = 25;   // Per Page
	include('../../pagination/pagination.php');
	$folder= 'masters/sourceofhire/view';
	$title = 'Source';

//$getData = mysql_query("SELECT * FROM `sourceofhire` WHERE `delete` = '0' AND `id` != '1' ORDER BY `name` ASC limit 100",$con) or die(mysql_error());
?>

<div id="myTitle">
<div class="title">View All Source</div>
<div class="strip">
<span>Dashboard</span>
<span>Source</span>
<span>View</span>
...
<?php if(in_array('a_MSOHire',$thisper)) 
{
?>
<button class="button blue" onclick="getModule('masters/sourceofhire/index','manipulateContent','viewContent','Source Of Hire')"> <i class="plus"></i>New Source</button>&nbsp;
<?php 
} 
?>
<?php if(in_array('d_MSOHire',$thisper)) 
{
?>
<button class="button red" onclick="deleteData('sourceofhire','sourceofhire')"> <i class="delete-icon"></i>Delete</button>&nbsp;
<?php 
} 
...
<?php if(in_array('u_MSOHire',$thisper)) 
{
?>
