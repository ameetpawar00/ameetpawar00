
<?php
include("../../include/conFig.php");
error_reporting(0);
$sql = "SELECT * FROM `companydetail` WHERE `delete` = '0' ";
$getData = mysql_query($sql,$con) or die(mysql_error());
$Num_Rows = mysql_num_rows($getData);
	$Per_Page = 25;   // Per Page
	include('../../pagination/pagination.php');
	$folder= 'masters/companydetails/view';
	$title = 'Company Details';

?>
<div id="myTitle">
<div class="title">Company Details</div>
<div class="strip">
<span>Dashboard</span>
<span>Company Details</span>
<span>View</span>
</div>
</div>



<table width="100%" cellpadding="0" cellspacing="0" id="headerTable">
<tr>
<td style="width:30%"></td>
<td style="width:70%" align="right">
<?php if(in_array('a_cmpd',$thisper)) 
{
?>
<button class="button blue" style="display:none" onclick="getModule('masters/companydetails/index','manipulateContent','viewContent','Employee')"> <i class="plus"></i>New</button>&nbsp;
<?php 
} 
?>
<?php if(in_array('d_cmpd',$thisper)) 
{
?>
<button class="button red" onclick="deleteData('companydetail','Company Details')"> <i class="delete-icon"></i>Delete</button>&nbsp;
<?php 
} 
?>&nbsp;<button class="button gray" onclick="getModule('masters/index','manipulateContent','viewContent','Setup');">
		<i class="back"></i>Back</button>
</td>
</tr>
</table>

<div style="height:350px;overflow:auto" id="mainDivId">
<table width="100%" cellpadding="5" cellspacing="0"  class="fetch" id="mytable">
<tr><th><input id="mainChk" name="Checkbox1" onclick="chkAll('chBx','mainChk')"type="checkbox" /></th>
	<th>Name</th>
	<th>Email</th>
	<th>Job Description</th>
</tr>
<?php
$i = 1;
$sql .=" order by `name` ASC LIMIT $Page_Start , $Per_Page";
$values = mysql_query($sql,$con)or die(mysql_error());
while($row =mysql_fetch_array($values))

{
?>
<tr  class="d<?php echo $i%2;?>" id="fetchrow<?php echo $i?>">
<td><input id="chBx<?php echo $i;?>" name="Checkbox1" type="checkbox" value="<?php echo $row[0];?>" /></td>
<?php if(in_array('u_cmpd',$thisper)) 
{
?>
<td class="link-blue" onclick="getModule('masters/companydetails/edit?id=<?php echo $row[0]?>&i=<?php echo $i?>','manipulateContent','viewContent','Company Detail')"><?php echo $row[1]?></td>
<?php 
} 
else
{
?>
<td ><?php echo $row[1]?></td>
<?php
}
?>

<td><?php echo $row[3]?></td>
<td><?php echo $row[9]?></td>
</tr>
<?php
$i++;
$Maxid = $row['id'];
$MaxI = $i;
}
?>
<input id="fetchData" name="Text1" style="display: none" type="text" value="<?php echo $Maxid.'--'.$MaxI;?>" />
</table>
<br/><br/><br/><br/><br/><br/><br/><br/><br/><br/>
</div>
<?php
include('../../pagination/pages.php');
?>
