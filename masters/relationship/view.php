...
$sql = "SELECT * FROM `relationship` WHERE `delete`='0' AND `id` != '1'";
$getData = mysql_query($sql,$con) or die(mysql_error());
$Num_Rows = mysql_num_rows($getData);
	$Per_Page = 25;   // Per Page
	include('../../pagination/pagination.php');
	$folder= 'masters/relationship/view';
	$title = 'Relationship';

$getData = mysql_query("SELECT * FROM `relationship` WHERE `delete` = '0' AND `id` != '1' ORDER BY `name` ASC",$con) or die(mysql_error());
?>
<div id="myTitle">
<div class="title">View All Relationship</div>
<div class="strip">
<span>Dashboard</span>
...
<?php if(in_array('a_MRelation',$thisper)) 
{
?>
<button class="button blue" onclick="getModule('masters/relationship/index','manipulateContent','viewContent','Relationship')"> <i class="plus"></i>New Relationship</button>&nbsp;
<?php 
} 
?>
<?php if(in_array('d_MRelation',$thisper)) 
{
?>
<button class="button red" onclick="deleteData('relationship','relationship')"> <i class="delete-icon"></i>Delete</button>&nbsp;
<?php 
} 
...
<?php if(in_array('u_MRelation',$thisper)) 
{
?>
