...
<tr>
 <th>Team Department<span>*</span></th>
 <td>
 	
    <select class="input drop-down large" name="req" id="team3" required="required">
    	<option value="">Select Department</option>
    	<?php $getdepartment = mysql_query("SELECT `id`, `name` FROM `department` WHERE `id`!=1 AND `delete`=0",$con) or die(mysql_error());
          while($row_department = mysql_fetch_array($getdepartment))
          {
          	$department_id=$row_department['id'];
          	$department_name=$row_department['name'];
        ?> 

        <option <?php if($row['department_id'] == $department_id){ echo "selected='selected'";} ?> value="<?php echo $department_id; ?>"><?php echo $department_name; ?></option> 
        <?php  } ?>	
    </select> 	  
...
<tr>
 <th>Self (Use only for Attendance)<span>*</span></th>
 <td>
    <select class="input drop-down large" name="req" id="team5" required="required">
    	<option value="0" <?php if($row['self_attendance'] == 0){ echo "selected='selected'";} ?>>Do not Show Self in Attendance </option>
    	<option value="1" <?php if($row['self_attendance'] == 1){ echo "selected='selected'";} ?>>Yes, Show Self in Attendance </option>
    </select> 	  
    
 </td>
</tr>
<tr><th>Description</th>
<td><textarea class="input huge" name="" cols="20" rows="2" style="width:48%;height:100px;" id="team4"><?php echo $row['desc'];?></textarea>
</td>
</tr>

<tr>
<td colspan="4" style="text-align:center">
<button class="button green" onclick="SaveData('masters/team/update?id=<?php echo $id; ?>&i=<?php echo $_GET['i'];?>','team','6','','','couResp','1')"><i class="save-icon"></i>Save</button>
