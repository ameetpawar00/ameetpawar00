...
$sql = "SELECT * FROM `reasonforleaving` WHERE `delete`='0' AND `id` != '1'";
$getData = mysql_query($sql,$con) or die(mysql_error());
$Num_Rows = mysql_num_rows($getData);
	$Per_Page = 25;   // Per Page
	include('../../pagination/pagination.php');
	$folder= 'masters/reasonforleaving/view';
	$title = 'Reason';

//$getData = mysql_query("SELECT * FROM `reasonforleaving` WHERE `delete` = '0' AND `id` != '1' ORDER BY `name` ASC limit 100",$con) or die(mysql_error());
?>

<div id="myTitle">
<div class="title">View All Reason</div>
<div class="strip">
<span>Dashboard</span>
...
<?php if(in_array('a_MRFLeav',$thisper)) 
{
?>
<button class="button blue" onclick="getModule('masters/reasonforleaving/index','manipulateContent','viewContent','Reason For Leaving')"> <i class="plus"></i>New Reason For Leaving</button>&nbsp;
<?php 
} 
?>
<?php if(in_array('d_MRFLeav',$thisper)) 
{
?>
<button class="button red" onclick="deleteData('reasonforleaving','reasonforleaving')"> <i class="delete-icon"></i>Delete</button>&nbsp;
<?php 
} 
...
<?php if(in_array('u_MRFLeav',$thisper)) 
{
?>
