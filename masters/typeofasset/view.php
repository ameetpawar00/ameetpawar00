...
$sql = "SELECT * FROM `typeofasset` WHERE `delete`='0'";
...
<?php if(in_array('a_MAssets',$thisper)) 
{
?>
<button class="button blue" onclick="getModule('masters/typeofasset/index','manipulateContent','viewContent','Asset')"> <i class="plus"></i>New Asset</button>&nbsp;
<?php 
} 
?>
<?php if(in_array('d_MAssets',$thisper)) 
{
?>
<button class="button red" onclick="deleteData('typeofasset','Asset')"> <i class="delete-icon"></i>Delete</button>&nbsp;
<?php 
} 
?>
...
<?php if(in_array('u_MAssets',$thisper)) 
{
?>
<td class="link-blue" onclick="getModule('masters/typeofasset/edit?id=<?php echo $row['id']?>&i=<?php echo $i?>','manipulateContent','viewContent','Assets')"><?php echo $row['name'] ?></td>
<?php 
} 
else
{
